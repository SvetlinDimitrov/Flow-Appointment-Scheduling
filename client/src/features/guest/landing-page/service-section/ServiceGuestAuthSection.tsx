...
const ServiceGuestAuthSection = ({
  title,
  description,
  buttonText,
}: ServiceGuestAuthSectionProps) => {
  ...
  const { data, isLoading, error, page, handlePageChange, setPage } = usePaginatedQuery<Service>(
  ...
  return (
    <MainWrapper>
      ...
      {isLoading && <ContainerLoader height={540} />}
      {!isLoading && data && (
        <>
          <CardsHolder>
            {data.content.map((service, index) => (
              <ServiceCard
                key={index}
                image={"https://placehold.co/150x150?text=No+Image"}
                alt={service.name}
                title={service.name}

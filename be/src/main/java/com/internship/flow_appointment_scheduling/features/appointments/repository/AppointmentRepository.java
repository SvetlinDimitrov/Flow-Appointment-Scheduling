...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
  ...
  @Query(
      "SELECT a FROM Appointment a " + "WHERE a.client.id = :userId " + "OR a.staff.id = :userId")
  ...
  List<Appointment> findAllByUserIdAndDate(
  ...
  @Query(
      "SELECT a FROM Appointment a "
          ...
          + "AND CAST(a.startDate AS date) = :date")
  List<Appointment> findAllByServiceIdAndDate(
      @Param("serviceId") Long serviceId, @Param("date") LocalDate date);
  ...
  @Query(
      "SELECT COUNT(a) "
          ...
          + "AND a.endDate > :startDate "
          + "AND (a.status = 'APPROVED' OR a.status = 'NOT_APPROVED')")
  int countAppointmentsInWorkspace(
